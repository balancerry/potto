...
import { clearPendingSignup, readPendingSignup } from '@/lib/auth/pending-signup';
import {
  friendlyAuthError,
  normalizeEmail,
  validateDisplayName,
...
type AuthContextValue = {
  ...
  updatePassword: (input: { password: string; confirmPassword: string }) => Promise<Result>;
...
function parseAuthUrl(url: string): { code?: string; access_token?: string; refresh_token?: string } {
  try {
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const establishPassword = useCallback(
    async (input: { password: string; confirmPassword: string; name?: string }): Promise<Result> => {
      const pErr = validatePassword(input.password);
      if (pErr) return { ok: false, error: pErr };
      const cErr = validatePasswordConfirm(input.password, input.confirmPassword);
      if (cErr) return { ok: false, error: cErr };

      const {
        data: { user },
      } = await supabase.auth.getUser();
      if (!user) return { ok: false, error: 'Please open the verification link again, then set your password.' };
      ...
      if (displayName) {
        const { error: profileErr } = await supabase
          ...
          .eq('id', user.id);
        if (profileErr) return { ok: false, error: friendlyAuthError(profileErr) };
      }
    ...
    () => ({
      ...
      async startSignup({ name, email, password, confirmPassword }) {
        ...
        if (!data.session) {
          ...
          if (signInErr) return { ok: false, error: friendlyAuthError(signInErr) };
        }
